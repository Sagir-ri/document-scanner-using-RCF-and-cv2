...
def adjust_poly_lr(optimizer, base_lr, iter, max_iter, power=0.9): #my poly_lr
...
def train(args, model, train_loader, optimizer, epoch, logger):
    ...
    for i, (image, label) in enumerate(train_loader):
        ...
        loss = loss / args.iter_size
        loss.backward()
        losses.update(loss.item() * args.iter_size, image.size(0))  # 恢复真实loss统计
        """
...
if __name__ == '__main__':
    ...
    if args.resume is not None:
        if osp.isfile(args.resume):
            ...
            try:
            #1 pytorch高版本torch.load引入安全性检查。故作出以下修改
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        #adjust_poly_lr(optimizer, args.lr, epoch * len(train_loader), args.max_epoch * len(train_loader)) ##polylr
        lr_scheduler.step() # will adjust learning rate ##steplr
