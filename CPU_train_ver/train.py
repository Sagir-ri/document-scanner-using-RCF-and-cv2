...
def train(args, model, train_loader, optimizer, epoch, logger):
    ...
    for i, (image, label) in enumerate(train_loader):
        ...
        outputs = model(image)
        loss = torch.zeros(1)  # Removed .cuda()
        for o in outputs:
...
def single_scale_test(model, test_loader, test_list, save_dir, logger):
    model.eval()
...
def multi_scale_test(model, test_loader, test_list, save_dir, logger):
    model.eval()
    if not osp.isdir(save_dir):
        os.makedirs(save_dir)
    scale = [0.5, 1, 1.5]
...
if __name__ == '__main__':
    ...
    # Removed GPU argument since we're using CPU
    ...
    # Removed CUDA environment settings
    ...
    # Removed .cuda() - model will run on CPU
    model = RCF(pretrained='vgg16convs.mat')
    ...
    if args.resume is not None:
        if osp.isfile(args.resume):
            ...
            checkpoint = torch.load(args.resume, map_location='cpu')  # Added map_location for CPU
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        logger.info('Performing training for epoch {}...'.format(epoch + 1))
        train(args, model, train_loader, optimizer, epoch, logger)
        save_dir = osp.join(args.save_dir, 'epoch%d-test' % (epoch + 1))
        single_scale_test(model, test_loader, test_list, save_dir, logger)
        multi_scale_test(model, test_loader, test_list, save_dir, logger)
        # Save checkpoint
