...
class RCF(nn.Module):
    ...
    # 新增方法：将双线性权重转移到正确的设备
    def to(self, device):
        """重写to方法，确保双线性权重也被正确转移到目标设备"""
        super().to(device)
        
        # 将双线性权重转移到正确的设备
        self.weight_deconv2 = self.weight_deconv2.to(device)
        self.weight_deconv3 = self.weight_deconv3.to(device)
        self.weight_deconv4 = self.weight_deconv4.to(device)
        self.weight_deconv5 = self.weight_deconv5.to(device)
        
        return self

    def cuda(self, device=None):
        """重写cuda方法，确保双线性权重也被转移到CUDA"""
        super().cuda(device)
        
        # 将双线性权重转移到CUDA
        self.weight_deconv2 = self.weight_deconv2.cuda(device)
        self.weight_deconv3 = self.weight_deconv3.cuda(device)
        self.weight_deconv4 = self.weight_deconv4.cuda(device)
        self.weight_deconv5 = self.weight_deconv5.cuda(device)
        
        return self

    def cpu(self):
        """重写cpu方法，确保双线性权重也被转移到CPU"""
        super().cpu()
        
        # 将双线性权重转移到CPU
        self.weight_deconv2 = self.weight_deconv2.cpu()
        self.weight_deconv3 = self.weight_deconv3.cpu()
        self.weight_deconv4 = self.weight_deconv4.cpu()
        self.weight_deconv5 = self.weight_deconv5.cpu()
